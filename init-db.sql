...
CREATE INDEX IF NOT EXISTS idx_users_dn ON users(dn);
...
CREATE INDEX IF NOT EXISTS idx_groups_dn ON groups(dn);
CREATE INDEX IF NOT EXISTS idx_groups_gid ON groups(gid_number);
...
CREATE INDEX IF NOT EXISTS idx_schema_class ON ldap_schema_cache(object_class);
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'readonly_monitor') THEN
